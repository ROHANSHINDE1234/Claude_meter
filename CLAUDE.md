...
- **The boot screen shows a QR code linking to the token-update page, not a fixed-duration text screen.** `DisplayManager::showQRCode()` uses ESP-IDF's bundled `qrcode` component (`espressif__qrcode`, header `qrcode.h`) — **not** a separately-installed Arduino library. A separate `QRCode` library (ricmoo) was tried first and removed: the ESP32 core's own `qrcode.h` is earlier on the include path and silently wins the collision, so installing another library of the same header name doesn't work regardless of what's in `libraries/`. `esp_qrcode_generate()`'s `display_func` callback has no context pointer, hence the static-member/scratch-state pattern in `DisplayManager` — this is deliberate, not an oversight to "clean up."
- **The QR code stays on screen until the first successful usage fetch, not for a fixed duration.** `claude_meter.ino` tracks `waitingForFirstFetch` and only calls `displayManager.tick()` (which would render the normal usage screen) once `usageManager.current().valid` is true. This means a bad/expired token leaves the QR showing indefinitely — intentional, since that's exactly when scanning it to paste a new token matters.
